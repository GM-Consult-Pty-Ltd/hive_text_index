...
class HashTagAnalyzer extends English {
  ...
  static Future<List<Token>> kFilterTokens(List<Token> tokens) async {
    ...
    for (final token in tokens) {
      if (token.termPosition == 0) {
        retVal.add(token);
      } else if (RegExp(r'\w+:\w+').allMatches(token.term).length == 1) {
        retVal.add(Token(token.term.split(':').first.toLowerCase(), token.n,
            token.termPosition, token.zone));
      }
    }
    return retVal;
  }

  static NGramRange? get kNGramRange => NGramRange(1, 3);

  static const kIndexName = 'hashtags';

  @override
  AsyncTermModifier get termFilter => (term, [zone]) async {
        return term.isEmpty ? null : term.toLowerCase();
      };
